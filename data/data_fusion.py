...
class FDDataset(Dataset):
    def __init__(self, mode, dataset_name, modality='color', fold_index='<NIL>', image_size=128, augment = None, balance = True, crop=True, gray=True):
    ...
    def set_mode(self, mode, fold_index):
        self.mode = mode
        self.fold_index = fold_index
        # print("set_mode:", self.mode)
        print("set_mode:",mode)
        # print('fold index set: ', fold_index)

        if self.mode == 'test' or self.mode=='dev':
            self.test_list = load_test_list(self.mode,self.dataset_name)
            self.num_data = len(self.test_list)
            print('set dataset mode: test')

        elif self.mode == 'val':
            self.val_list = load_val_list(self.dataset_name)
            self.num_data = len(self.val_list)
            print('set dataset mode: val')

        elif self.mode == 'train':
            print("self.train_list = load_train_list(self.dataset_name)")
            self.train_list = load_train_list(self.dataset_name)
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            if self.balance:
                if random.randint(0,1)==0:
                    ...
                    # print("self.train_list[0]:",len(self.train_list[0]))
                    # print("self.train_list[0]:",self.train_list[0])
                else:
                    tmp_list = self.train_list[1]
                    # print("self.train_list[1]:",len(self.train_list[1]))
        ...
        #read image
        color = cv2.imread(os.path.join(DATA_ROOT[self.mode], color),1)
        depth = cv2.imread(os.path.join(DATA_ROOT[self.mode], depth),1)
        ir = cv2.imread(os.path.join(DATA_ROOT[self.mode], ir),1)

        #cropped
        if self.crop==True:
            color  = self.crop_image_from_gray(color)
            depth = self.crop_image_from_gray(depth)
            ir  = self.crop_image_from_gray(ir )

        #RGB2Gray
        if self.gray==True:
            imgray = cv2.cvtColor(color, cv2.COLOR_BGR2GRAY)
            color= np.stack([imgray, imgray, imgray], axis=-1)
        ...
        if self.mode == 'train':
            ...
            image = np.concatenate([color.reshape([self.image_size, self.image_size, 3]),
                                   ...
                                   axis=2)
            # print("con image.shape:", image.shape)
            ...
            # print("tran image.shape:", image.shape)
            ...
            image = image / 255.0
            # print("image_size in val:", image.size)
            # print("image_shape in val:", image.shape)
            label = int(label)
            return torch.FloatTensor(image), torch.LongTensor(np.asarray(label).reshape([-1]))
